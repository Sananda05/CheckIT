...
from HTRmodel.dataloader_iam import DataLoaderIAM,Batch
from HTRmodel.models import Model, DecoderType
from HTRmodel.preprocessor import Preprocessor

from PdfConversion.models import HandWrittenFiles
from django.contrib.auth.models import User


class FilePaths:
    """Filenames and paths to data."""
    fn_char_list = './HTRmodel/model/charList.txt'
    fn_summary = './HTRmodel/model/summary.json'
    fn_corpus = './HTRmodel/data/corpus.txt'
...
def validate(model: Model, loader: DataLoaderIAM, line_mode: bool) -> Tuple[float, float]:
    ...
    while loader.has_next():
        ...
        for i in range(len(recognized)):
            ...
            num_word_total += 1
            dist =edit_distance.eval(recognized[i], batch.gt_texts[i])
            num_char_err += dist
...
def getPdf(file,user):
    """Main function."""
    parser = argparse.ArgumentParser()
    # path = str(file.pdf)
    # add = "media/"
    # imgfile = add + path
    # imgfile = r"media\files\img3.jpeg"
    print("-------Print-------")
    print(file)
    ...
    # parser.add_argument('--img_file', help='Image used for inference.', type=Path, default=imgfile)
    # parser.add_argument('--early_stopping', help='Early stopping epochs.', type=int, default=25)
    # parser.add_argument('--dump', help='Dump output of NN to CSV file(s).', action='store_true')
    # args = parser.parse_args()
    args = argparse.Namespace(mode='infer', decoder='bestpath', batch_size=100, data_dir=None, fast=False, line_mode=True, img_file=file, early_stopping=25, dump=False)
